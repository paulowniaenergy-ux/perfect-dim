...
import { formatPrice, getInfrastructure, getPropertyVideos, infrastructureOptions, statusLabels, type Property } from '@/lib/property-types';
...
export function PropertyDetailClient({ property, relatedProperties = [] }: { property: Property; relatedProperties?: Property[] }) {
  ...
  return (
    <main className="bg-[#f6f1e8]"><SiteHeader />
      ...
      {relatedProperties.length > 0 && <section className="border-t border-[#173326]/10 bg-[#ece5d6] py-20 lg:py-24"><div className="pd-container"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="pd-eyebrow">Поруч із цим об’єктом</p><h2 className="mt-3 text-4xl text-[#173326] sm:text-5xl">Інші будинки, які варто подивитися</h2></div><a href="/catalog" className="inline-flex items-center gap-2 text-sm font-semibold text-[#173326] underline decoration-[#b99751] underline-offset-8">Увесь каталог <ChevronRight className="size-4" /></a></div><div className="mt-10 grid gap-6 lg:grid-cols-3">{relatedProperties.map((item) => <PropertyCard key={item.id} property={item} />)}</div></div></section>}
      <SiteFooter />

...
import { formatPrice, statusLabels, type Property } from '@/lib/properties';
import { loadProperties } from '@/lib/property-store';

export function PropertyDetailClient({ seed }: { seed: Property }) {
  const [property, setProperty] = useState(seed);
  useEffect(() => {
    const current = loadProperties().find((item) => item.slug === seed.slug);
    ...
    <main className="bg-[#f6f1e8]"><SiteHeader />
      <section className="pd-container py-8 lg:py-12">
        <div className="mb-7 flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
          <div><a href="/catalog" className="text-xs font-semibold uppercase tracking-[.15em] text-[#9a783b]">← Назад до каталогу</a><h1 className="mt-4 text-5xl leading-none text-[#173326] sm:text-7xl">{property.title}</h1><p className="mt-4 flex items-center gap-2 text-sm text-[#647368]"><MapPin className="size-4 text-[#b99751]" /> {property.address} · {property.distance} км від Луцька</p></div>
          <div className="lg:text-right"><span className={`inline-flex px-4 py-2 text-xs font-bold uppercase tracking-[.15em] text-white ${statusTone}`}>{statusLabels[property.status]}</span><p className="mt-4 text-3xl font-semibold text-[#173326]">{formatPrice(property.price)}</p></div>
        </div>
        ...
        <div className="grid gap-3 lg:grid-cols-[1.7fr_1fr]">
          <img src={property.images[0]} alt={`${property.title} — головний вигляд`} className="aspect-[16/10] h-full w-full object-cover" />
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-1"><img src={property.images[1]} alt={`${property.title} — додатковий вигляд`} className="h-full min-h-44 w-full object-cover" /><img src={property.images[2]} alt={`${property.title} — ділянка`} className="h-full min-h-44 w-full object-cover" /></div>
        </div>
      </section>
      ...
      <section className="pd-container grid gap-12 pb-20 pt-6 lg:grid-cols-[1.35fr_.65fr] lg:pb-28">
        <div>
          <div className="grid grid-cols-2 gap-px bg-[#173326]/12 sm:grid-cols-4">
            <Spec icon={Maximize2} value={`${property.houseArea} м²`} label="Площа будинку" /><Spec icon={BedDouble} value={`${property.rooms}`} label="Кімнат" /><Spec icon={LandPlot} value={`${property.landArea} сот.`} label="Ділянка" /><Spec icon={Maximize2} value={`${property.floors}`} label="Поверхів" />
          </div>
          <div className="mt-12"><p className="pd-eyebrow">Про будинок</p><h2 className="mt-4 text-4xl text-[#173326]">Простір для вашого ритму життя</h2><p className="mt-6 max-w-3xl text-base leading-8 text-[#586a5f]">{property.description}</p></div>
